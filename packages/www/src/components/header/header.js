						...
						<HeaderFlyout>
							<HeaderLink to="/coffees">
								<div className="p-2 flex items-center space-x-4 rounded-lg">
									...
									<div>
										<p>Coffee</p>
										<p className="font-normal opacity-75 text-sm">Lorem ipsum dolor sit amet</p>
									</div>
								</div>
							</HeaderLink>
							<HeaderLink>
	...
	<div className="z-30 absolute -ml-4 mt-3 w-screen max-w-md">
		<div className="border-8 border-secondary bg-primary overflow-hidden shadow-xs">
			<div className="shadow-lg">
				<div className="relative grid p-2 lg:p-4 gap-1 lg:gap-2">{children}</div>
			</div>
		</div>
	</div>
);
...
export const HeaderLg = ({ children }) => (
	<div>
		<div className="flex justify-between items-center p-6 space-x-8">{children}</div>
	</div>
);

export const HeaderSm = ({ children, alwaysShow }) => (
	<div
		className={`
